...
export function carry_(creep_: Creep): void {
    ...
    if (creep_.memory.fillingConstruction && !Game.getObjectById(creep_.memory.fillingConstruction as Id<StructureExtension | StructureSpawn>).store.getFreeCapacity('energy')) creep_.memory.fillingConstruction = null;//要填的建筑容量满了就重置
    if (!Game.rooms[creep_.memory.belong]) return
    creep_.workstate('energy')
    ...
    if (creep_.memory.working) {
        ...
        if (a == OK) {
            ...
            if (!target1) {
                target1 = creep_.pos.findClosestByRange(FIND_STRUCTURES, {
        ...
        else { if (container.store.getUsedCapacity('energy') > creep_.store.getFreeCapacity()) { creep_.withdraw(container, 'energy'); creep_.memory.fillingConstruction = null } }
    }
}
