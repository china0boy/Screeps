...
export function squadFrameWork(squardID: string): void {
    ...
    if (attack_flag) {
        if (attack_flag.pos.lookFor(LOOK_STRUCTURES).length <= 0) attack_flag.remove()
            ...
            let Attackdirection = SquadAttackDirection(Data.creepData)
    ...
    /* 如果有红色旗帜，优先攻击红色旗帜周围爬  */
    var RedFlag = SquadColorFlagRange(squadData, COLOR_RED)
    if (RedFlag) {
        let attackCreep = RedFlag.pos.findClosestByPath(FIND_HOSTILE_CREEPS, {
            filter: (creep) => {
                return !Memory.whitesheet.includes(creep.owner.username)
            }
        })
        if (attackCreep) {
            let Attackdirection = SquadAttackDirection(Data.creepData)
            if (SquadPosDirection(squadData, attackCreep.pos) != null && Attackdirection != SquadPosDirection(squadData, attackCreep.pos)) {
                if (!isInArray(['↙', '↗', '↘', '↖'], SquadPosDirection(squadData, attackCreep.pos))) {
                    SquadAttackOrient(Attackdirection, SquadPosDirection(squadData, attackCreep.pos), squadData)
                    return
                }
            }
            if (!squadNear(squadData, attackCreep.pos)) {
                squadMove(squadData, attackCreep.pos, 1)
            }
        }
        else {
            if (!squadNear(squadData, RedFlag.pos)) {
                squadMove(squadData, RedFlag.pos, 0)
            }
        }
    }
