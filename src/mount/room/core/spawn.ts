    ...
    public SpawnExecution(): void {
        ...
        for (let sID of this.memory.StructureIdData.spawn as string[]) {
            ...
            if (allEnergy >= CalculateEnergy(body)) {
                let result = thisSpawn.spawnCreep(body, `人${timestr}类${mark}高质量`, { memory: thisMem })//${mark}·${timestr}${randomStr} ${timestr}----${mark}  ${timestr}|${mark}${randomStr}  ${randomStr}__${mark}${timestr}  人${timestr}类${mark}高质量  ${randomStr}${mark}${timestr}
                if (result == OK) {
