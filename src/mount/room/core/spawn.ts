...
export default class RoomCoreSpawnExtension extends Room {
    ...
    public SpawnConfigInit(): void {
        ...
        /* 初始化 */
        for (let role in RoleData)//遍历爬的注册列表
        {
    ...
    public SpawnExecution(): void {
        ...
        for (let sID of this.memory.StructureIdData.spawn as string[]) {
            ...
            if (allEnergyCapacity < bodyEnergy) adaption_body(body, allEnergyCapacity)
            /* 对具备自适应属性的爬虫进行自适应 */
            let adaption = false
            ...
            if (allEnergy >= CalculateEnergy(body)) {
                let result = thisSpawn.spawnCreep(body, `${mark}·${timestr}${randomStr}`, { memory: thisMem })//${mark}·${timestr}${randomStr} ${timestr}----${mark}  ${timestr}|${mark}${randomStr}  ${randomStr}__${mark}${timestr}  人${timestr}类${mark}高质量  ${randomStr}${mark}${timestr}
                if (result == OK) {
