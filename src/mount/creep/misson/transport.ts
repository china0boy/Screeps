        ...
        if (this.memory.working) {
            ...
            if (storage_.store['energy'] >= this.store.getCapacity())
                if (this.withdraw_(storage_, 'energy') == OK) {//拿能量成功最新的最近填充建筑
                    ...
                    if (terminal_ && terminal_.store.getUsedCapacity('energy') >= this.store.getCapacity()) if (this.withdraw_(terminal_, 'energy') == OK) this.memory.fillingConstruction = this.pos.getClosestStore().id;
                }
        }
    }
